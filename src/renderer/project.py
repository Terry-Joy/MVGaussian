...
class UVProjection():
    def __init__(self, texture_size=96, render_size=64, sampling_mode="nearest", channels=3, device=None):
        ...
        self.lights = AmbientLights(ambient_color=(
            (1.0,)*channels,), device=self.device)
    ...
    def construct_colmap(self, obj_path, save_path):
        ...
        camera_txt_path = os.path.join(sparse_path, "cameras.txt")
    ...
    def process_and_save_mesh_as_pointcloud(self, obj_path, save_path):
        ...
        # Center the mesh to the origin using the average of max and min coordinates
        max_coords = verts.max(dim=0)[0]
        min_coords = verts.min(dim=0)[0]
        center = (max_coords + min_coords) / 2
        verts -= center

        # Normalize the mesh to the range [-1, 1]
        scale = verts.abs().max()
        verts /= scale
    ...
    def save_image_txt(self, save_path):
        with open(save_path, 'a') as file:
            for i in range(len(self.cameras)):
                img_params = f"{i + 1} "
                # qvec = rotmat2qvec(self.cameras[i].R)
                w, x, y, z = rotation_matrix_to_quaternion(self.cameras[i].R)
                tvec = self.cameras[i].T[0]
                # img_params += f"{qvec.w} {qvec.x} {qvec.y} {qvec.z} "
                img_params += f"{w} {x} {y} {z} "
                img_params += f"{tvec[0]} {tvec[1]} {tvec[2]} "
                img_params += "1 "
                img_params += f"{i+1:05d}.jpg\n\n"
    ...
    def uv_unwrap(self, mesh):
        ...
        vt = torch.from_numpy(vt_np.astype(np.float32)).type(
            verts_list.dtype).to(mesh.device)
        ft = torch.from_numpy(ft_np.astype(np.int64)).type(
            faces_list.dtype).to(mesh.device)

        new_map = torch.zeros(
            self.target_size+(self.channels,), device=mesh.device)
        new_tex = TexturesUV(
            [new_map],
            [ft],
            [vt],
    ...
    def disconnect_faces(self):
        ...
        self.mesh_d = Meshes(verts_disconnect_list,
                             faces_uvs_list, mesh.textures)
        return self.mesh_d
    ...
    def calculate_cos_angle_weights(self, cos_angles, fill=True, channels=None):
        ...
        for i in range(len(self.cameras)):
            ...
            optimizer.zero_grad()
            zero_tex = TexturesUV([zero_map], self.mesh.textures.faces_uvs_padded(
            ), self.mesh.textures.verts_uvs_padded(), sampling_mode=self.sampling_mode)
    ...
    def decode_view_normal(self, normals):
        ...
        normals_view[:, :, :, 2] *= -1
        normals = (normals_view[..., 0:3]+1) * normals[..., 3:] / 2 + torch.FloatTensor(
            ((((0.5, 0.5, 1))))).to(self.device) * (1 - normals[..., 3:])
    ...
    def decode_normalized_depth(self, depths, batched_norm=False):
        ...
        print('view_z shape is: ', view_z.shape)
    ...
    def calculate_visible_triangle_mask(self, channels=None, image_size=(512, 512)):
        ...
        for i in range(len(pix2face_list)):
            ...
            valid_faceid = valid_faceid[1:] if valid_faceid[0] == - \
                1 else valid_faceid
            mask = torch.isin(
                uv_pix2face[0], valid_faceid, assume_unique=False)
            # uv_pix2face[0][~mask] = -1
            triangle_mask = torch.ones(
                self.target_size+(1,), device=self.device)
    ...
    def bake_texture(self, views=None, main_views=[], cos_weighted=True, channels=None, exp=None, noisy=False, generator=None):
        ...
        tmp_mesh = self.mesh
        bake_maps = [torch.zeros(self.target_size+(views[0].shape[2],),
                                 device=self.device, requires_grad=True) for view in views]
        optimizer = torch.optim.SGD(bake_maps, lr=1, momentum=0)
        optimizer.zero_grad()
        loss = 0
        for i in range(len(self.cameras)):
            bake_tex = TexturesUV([bake_maps[i]], tmp_mesh.textures.faces_uvs_padded(
            ), tmp_mesh.textures.verts_uvs_padded(), sampling_mode=self.sampling_mode)
            tmp_mesh.textures = bake_tex
            images_predicted = self.renderer(
                tmp_mesh, cameras=self.cameras[i], lights=self.lights, device=self.device)
        ...
        for i in range(len(bake_maps)):
            ...
            if noisy:
                noise = torch.rand(
                    weight.shape[:-1]+(1,), generator=generator).type(weight.dtype).to(weight.device)
                weight *= noise
        ...
        bake_tex = TexturesUV([baked], tmp_mesh.textures.faces_uvs_padded(
        ), tmp_mesh.textures.verts_uvs_padded(), sampling_mode=self.sampling_mode)
        tmp_mesh.textures = bake_tex
